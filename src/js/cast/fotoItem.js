...
export default class FotoItem {
  ...
  // calculate the position/size of both the list item and reveal element
  calcBounds() {
    this.bounds = {
  ...
  render() {
    ...
    this.animatableProperties.tx.current = Math.abs(
      mousepos.x - this.bounds.el.left
    );
    this.animatableProperties.ty.current =
      this.mouseArea === "up"
        ? Math.abs(mousepos.y - this.bounds.el.top)
        : Math.abs(mousepos.y - this.bounds.el.top) - this.bounds.reveal.height;
    // new rotation value
